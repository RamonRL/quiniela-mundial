...
type RevealState = { chosenCode: string; isCorrect: boolean };
...
export function AdivinaLaBanderaClient({ myIdentityKey, myBestScore }: Props) {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      if (left <= 0) {
        ...
        if (current.kind === "playing") finishGame(current);
      }
    }, 250);
  ...
  const beginGame = useCallback(async () => {
    ...
    try {
      ...
      preloadFlags(res.rounds.slice(0, 8));
      setPhase({
        kind: "playing",
        rounds: res.rounds,
        token: res.token,
  ...
  const advance = useCallback(() => {
    setPhase((current) => {
      ...
      preloadFlags(current.rounds.slice(nextIndex, nextIndex + 2));
  ...
  const handleChoose = (option: RoundOption) => {
    setPhase((current) => {
      ...
      const isCorrect = option.code === round.correctCode;
      const newChoices = [
        ...current.choices,
        { roundId: round.roundId, chosenCode: option.code },
      ];
      if (revealTimerRef.current != null) window.clearTimeout(revealTimerRef.current);
      ...
      return {
        ...
        reveal: { chosenCode: option.code, isCorrect },
      };
    });
  };
...
function PlayPanel({
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex flex-col items-center">
          ...
          <span className="font-display text-3xl tabular leading-none text-[var(--mj-text)] sm:text-4xl">
            ...
            <span className="text-2xl text-[var(--mj-text-dim)]">
      ...
      {/* Bandera con frame "viewfinder" */}
      <div
        key={round.roundId}
        className={cn(
          "mj-slam-in relative mx-auto flex aspect-[4/3] w-full max-w-md items-center justify-center overflow-hidden rounded-xl border-2 bg-[var(--mj-bg-3)] p-8 transition-colors",
          reveal == null
      ...
      >
        ...
        <img
          src={worldFlagUrl(round.flagCode)}
          alt=""
      ...
      {/* Opciones · solo nombre, sin chip de bandera (sería un spoiler) */}
      <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 sm:gap-3">
        {round.options.map((opt) => {
          const isChosen = reveal?.chosenCode === opt.code;
          const isCorrect = opt.code === round.correctCode;
          let stateClass = "";
          if (reveal != null) {
            if (isChosen) {
          ...
          return (
            <button
              ...
              className={cn(
                "group flex items-center justify-between gap-3 rounded-lg border-2 px-4 py-4 text-left transition disabled:cursor-default",
                reveal == null
...
function preloadFlags(rounds: Round[]) {
  if (typeof window === "undefined") return;
  for (const r of rounds) {
    const img = new window.Image();
    img.src = worldFlagUrl(r.flagCode);
  }
}
